...
func main() {
	...
	const producers = 4
	const consumers = 4
	before := time.Now() // tiden just nu

	ch := make(chan string) // skapar kanal

	wgp := new(sync.WaitGroup) // synkar alla kanaler
	wgp_new := new(sync.WaitGroup)

	wgp.Add(producers)
	wgp_new.Add((consumers))

	for i := 0; i < producers; i++ {
		go Produce("p"+strconv.Itoa(i), strings/producers, ch, wgp)
	}
	for i := 0; i < consumers; i++ {
		go Consume("c"+strconv.Itoa(i), ch, wgp_new)
...
func Produce(id string, n int, ch chan<- string, wg *sync.WaitGroup) {
	for i := 0; i < n; i++ {
		...
		ch <- id + ":" + strconv.Itoa(i) // string conversion, antal p
...
func Consume(id string, ch <-chan string, wg *sync.WaitGroup) {
	for s := range ch {
		fmt.Println(id, "received", s) // id = c, s = p
